...
def chart_datetime(container, chart_data):
    ...
    title = html.escape(chart_data.get('title', ''))
    x_title = html.escape(chart_data.get('x_title', ''))
    y_title = html.escape(chart_data.get('y_title', ''))
    # name = html.escape(chart_data.get('name', ''))
    x_type = html.escape(chart_data.get('x_type', 'datetime'))
    tooltips = {}
    ...
    for i in range(1, 10):
        ...
        if f'data{i}' in chart_data and f'x_keyword{i}' in chart_data and f'y_keyword{i}' in chart_data:
            ...
            x_keyword = html.escape(chart_data.get(f'x_keyword{i}'))
            y_keyword = html.escape(chart_data.get(f'y_keyword{i}'))
            chart_type = html.escape(chart_data.get(f'type{i}', 'line'))
            name = html.escape(chart_data.get(f'name{i}', ''))
            visible = bool(chart_data.get(f'visible{i}', True))
            color = html.escape(chart_data.get(f'color{i}', ''))
            ...
            for row in data:
                if type(row[x_keyword]) is datetime.datetime:
                    ...
                    x = row[x_keyword]
                y = row[y_keyword]
                js_data += f'[{html.escape(str(x))}, {html.escape(str(y))}],'
            js_data = '[' + js_data + ']'
